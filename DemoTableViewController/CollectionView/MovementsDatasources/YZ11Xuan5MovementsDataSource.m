...
@interface YZ11Xuan5MovementsDataSource ()

@property(nonatomic, copy) NSArray *allIndexData;

@end
@implementation YZ11Xuan5MovementsDataSource
...
- (void)loadDataWithHandle:(LoadDataCompleteHandle)complete {
    ...
    [YZMovementsConvertTool convert11xuan5JsonToModelsWithFileName:@"11xuan5" complete:^(NSArray *leftTitles, NSArray *topTitles, NSArray *allDatas) {
        ...
        weakSelf.topTitles = topTitles[0];
...
- (CGSize)itemSizeForMovementsView:(YZMovementsView *)view {
    if ([self numberOfColumnsForMovementsView:nil] == 0) {
        return CGSizeMake(kScreenWidth / 12.f, kScreenWidth / 12.f);
    }
    return CGSizeMake(kScreenWidth / [self numberOfColumnsForMovementsView:nil], kScreenWidth / 12);
}
