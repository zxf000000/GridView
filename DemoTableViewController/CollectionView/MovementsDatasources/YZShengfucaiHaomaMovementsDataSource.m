...
@interface YZShengfucaiHaomaMovementsDataSource ()

@property(nonatomic, copy) NSArray *allIndexData;

@end

@implementation YZShengfucaiHaomaMovementsDataSource


- (void)loadDataWithHandle:(LoadDataCompleteHandle)complete {
    __weak typeof(self) weakSelf = self;
    [YZMovementsConvertTool convertShengfucaiJsonToModelsWithFileName:@"shenfucaihaoma" complete:^(NSArray *leftTitles, NSArray *topTitles, NSArray *allDatas) {
...
- (void)loadSaiguoDataWithHandle:(LoadDataCompleteHandle)complete {
    __weak typeof(self) weakSelf = self;
    [YZMovementsConvertTool convertShengfucaiSaiguoJsonToModelsWithFileName:@"shengfusaiguo" complete:^(NSArray *leftTitles, NSArray *topTitles, NSArray *allDatas) {
        weakSelf.leftTitles = leftTitles;
        weakSelf.datas = allDatas[0];
        weakSelf.allIndexData = allDatas;
        weakSelf.topTitles = topTitles;
...
- (CGSize)itemSizeForMovementsView:(YZMovementsView *)view {
    ...
    return CGSizeMake(kScreenWidth / 17.f, kScreenWidth / 17.f);
}
